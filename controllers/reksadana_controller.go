...
func (server *Server) GetAllReksadana(w http.ResponseWriter, r *http.Request) {
	...
	posts, err := post.FindAllReksadana(server.DB)
	if err != nil {
...
func (server *Server) GetReksadana(w http.ResponseWriter, r *http.Request) {
	...
	postReceived, err := post.FindReksadanaByID(server.DB, pid)
	if err != nil {
		responses.ERROR(w, http.StatusInternalServerError, err)
		return
	}
...
func (server *Server) UpdateReksadana(w http.ResponseWriter, r *http.Request) {
	...
	post := models.Reksadana{}
	err = server.DB.Debug().Model(models.Reksadana{}).Where("id = ?", pid).Take(&post).Error
	if err != nil {
	...
	postUpdated, err := postUpdate.UpdateAReksadana(server.DB)
...
func (server *Server) DeleteReksadana(w http.ResponseWriter, r *http.Request) {
